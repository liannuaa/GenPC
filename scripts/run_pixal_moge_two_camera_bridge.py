...
def main() -> None:
    ...
    parser.add_argument(
        "--point-uv-origin", choices=("auto", "top-left", "bottom-left"), default="auto",
        help="Normalized Camera-1 UV origin; auto infers it from foreground support.",
    )
    ...
    if args.point_uv_origin == "auto":
        flip_y, uv_origin_scores = infer_point_uv_flip_y(point_uv, source_mask)
        uv_origin = "bottom-left" if flip_y else "top-left"
    else:
        uv_origin = args.point_uv_origin
        flip_y = uv_origin == "bottom-left"
        uv_origin_scores = None
    matches, match_info = transferred_partial_to_moge_matches(
        point_uv, source_mask.shape, camera1_to_camera2, moge_pixels,
        max_pixel_distance=args.max_pixel_distance, flip_y=flip_y,
    )
    ...
    record = {
        ...
        "point_uv_contract": {
            "requested_origin": args.point_uv_origin,
            "resolved_origin": uv_origin,
            "flip_y": bool(flip_y),
            "foreground_support_scores": uv_origin_scores,
        },
