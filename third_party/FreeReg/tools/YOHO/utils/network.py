...
class PartI_network(nn.Module):
    def __init__(self, cfg):
        ...
        self.Nei_in_SO3=torch.from_numpy(np.load(f'{self.cfg.SO3_related_files}/Nei_Index_in_SO3_ordered_13.npy').astype(np.int64).reshape([-1])).cuda()    #nei 60*12 readin
...
class PartI_train(nn.Module):
    def __init__(self, cfg):
        ...
        self.R_index_permu=torch.from_numpy(np.load(f'{self.cfg.SO3_related_files}/60_60.npy').astype(np.int64)).cuda()
