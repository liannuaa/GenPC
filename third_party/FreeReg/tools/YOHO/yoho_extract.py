...
group_file = str(Path(__file__).resolve().parent / 'utils' / 'group_related')

# fake config
args = argparse.Namespace(SO3_related_files=group_file)
...
class yoho_extractor():
    ...
    def _load_model(self):
        pth = torch.load(self.yoho_ckpt, weights_only=False)['network_state_dict']
